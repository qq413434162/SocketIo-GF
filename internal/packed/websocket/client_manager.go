...
// AddUsers 添加用户
func (manager *ClientManager) AddUsers(userId uint64, client *Client) {
	manager.UserLock.Lock()
	defer manager.UserLock.Unlock()
	manager.Users[userId] = client
}
...
func (manager *ClientManager) EventLogin(login *login) {
	...
	if manager.InClient(client) {
		manager.AddUsers(login.UserId, login.Client)
	}
}
